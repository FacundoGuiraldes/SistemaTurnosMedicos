...
* **ConcreteCreator (`TurnoTelemedicinaFactory`):** Resuelve la problemática específica de la atención clínica integrada de alta complejidad médica. Implementa de manera nativa la interacción y autenticación segura con la API del proveedor de salud para generar el `tokenPlataformaMedica` confidencial exigido por la regulación del sanatorio.
* **Product (`Turno`):** Diseñado como clase abstracta en lugar de interfaz pura porque aloja atributos estructurales del dominio compartidos por todos los tipos de turnos (`fecha`, `hora`, `especialidad`) y define la firma polimórfica del método template `ejecutarProtocolo()`.
* **ConcreteProducts (`TurnoPresencial`, `TurnoVirtual`, `TurnoTelemedicina`):** Especializaciones del dominio que incorporan sus propios atributos de estado complementarios y específicos (`nroConsultorio`, `urlReunion`, y `tokenPlataformaMedica` respectivamente) para ejecutar sus comportamientos polimórficos de atención al paciente.
